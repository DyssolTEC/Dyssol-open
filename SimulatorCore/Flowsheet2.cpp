...
CFlowsheet2::CFlowsheet2(CModelsManager& _modelsManager, const CMaterialsDatabase& _materialsDB) :
...
bool CFlowsheet2::IsEmpty() const
{
...
size_t CFlowsheet2::GetUnitsNumber() const
{
	return m_units.size();
}

CUnitContainer* CFlowsheet2::AddUnit(const std::string& _key)
{
...
void CFlowsheet2::DeleteUnit(const std::string& _key)
{
	// check if it exists
	...
	SetTopologyModified(true);
	// delete unit
	VectorDelete(m_units, [&](const std::unique_ptr<CUnitContainer>& u) { return u->GetUniqueID() == _key; });
}

void CFlowsheet2::ShiftUnit(const std::string& _key, EDirection _direction)
{
	const size_t index = VectorFind(m_units, [&](const auto& u) { return u->GetUniqueID() == _key; });
...
const CUnitContainer* CFlowsheet2::GetUnit(const std::string& _key) const
{
	for (const auto& unit : m_units)
		if (unit->GetUniqueID() == _key)
			return unit.get();
	return nullptr;
}

CUnitContainer* CFlowsheet2::GetUnit(const std::string& _key)
{
	return const_cast<CUnitContainer*>(static_cast<const CFlowsheet2&>(*this).GetUnit(_key));
}

std::vector<const CUnitContainer*> CFlowsheet2::GetAllUnits() const
{
...
std::vector<CUnitContainer*> CFlowsheet2::GetAllUnits()
{
...
size_t CFlowsheet2::GetStreamsNumber() const
{
	return m_streams.size();
}

CStream* CFlowsheet2::AddStream(const std::string& _key)
{
...
void CFlowsheet2::DeleteStream(const std::string& _key)
{
	// check if it exists
	...
	SetTopologyModified(true);
	// delete stream
	VectorDelete(m_streams, [&](const std::unique_ptr<CStream>& s) { return s->GetKey() == _key; });
}

void CFlowsheet2::ShiftStream(const std::string& _key, EDirection _direction)
{
...
const CStream* CFlowsheet2::GetStream(const std::string& _key) const
{
...
CStream* CFlowsheet2::GetStream(const std::string& _key)
{
	return const_cast<CStream*>(static_cast<const CFlowsheet2&>(*this).GetStream(_key));
}

std::vector<const CStream*> CFlowsheet2::GetAllStreams() const
{
...
std::vector<CStream*> CFlowsheet2::GetAllStreams()
{
...
bool CFlowsheet2::DetermineCalculationSequence()
{
	...
	for (auto& partition : iUnits)
	{
		...
		for (size_t i : partition)
			unitsKeys.back().push_back(m_units[i]->GetUniqueID());
	}
...
void CFlowsheet2::SetTopologyModified(bool _flag)
{
	m_topologyModified = _flag;
}

const CCalculationSequence* CFlowsheet2::GetCalculationSequence() const
{
	return &m_calculationSequence;
}

CCalculationSequence* CFlowsheet2::GetCalculationSequence()
{
	return &m_calculationSequence;
}

size_t CFlowsheet2::GetCompoundsNumber() const
{
	return m_compounds.size();
}

void CFlowsheet2::AddCompound(const std::string& _key)
{
...
void CFlowsheet2::RemoveCompound(const std::string& _key)
{
	// check if exists
	if (!VectorContains(m_compounds, _key)) return;
...
std::vector<std::string> CFlowsheet2::GetCompounds() const
{
	return m_compounds;
}

size_t CFlowsheet2::GetOverallPropertiesNumber() const
{
	return m_overall.size();
}

void CFlowsheet2::AddOverallProperty(EOverall _property, const std::string& _name, const std::string& _units)
{
	// check if already exists
...
void CFlowsheet2::RemoveOverallProperty(EOverall _property)
{
	// check if exists
	if (!VectorContains(m_overall, [&](const auto& o) { return o->type == _property; })) return;
...
std::vector<SOverallDescriptor> CFlowsheet2::GetOveralProperties() const
{
	return m_overall;
}

size_t CFlowsheet2::GetPhasesNumber() const
{
	return m_phases.size();
}

void CFlowsheet2::AddPhase(EPhase _phase, const std::string& _name)
{
	// check if already exists
...
void CFlowsheet2::RemovePhase(EPhase _phase)
{
	// check if exists
	if (!VectorContains(m_phases, [&](const auto& p) { return p->state == _phase; })) return;
	...
	// remove from the list of overall properties
	VectorDelete(m_phases, [&](const auto& p) { return p->type == _phase; });
}

std::vector<SPhaseDescriptor> CFlowsheet2::GetPhases() const
{
	return m_phases;
}

bool CFlowsheet2::IsPhaseDefined(EPhase _phase)
{
	return VectorContains(m_phases, [&](const auto& p) { return p->type == _phase; });
}

std::string CFlowsheet2::Initialize()
{
...
void CFlowsheet2::SetStreamsToPorts()
{
	for (auto& unit : m_units)
...
std::string CFlowsheet2::CheckPortsConnections()
{
...
void CFlowsheet2::ClearSimulationResults()
{
	for (auto& stream : m_streams)
...
void CFlowsheet2::UpdateDistributionsGrid()
{
	for (auto& stream : m_streams)
...
void CFlowsheet2::UpdateCacheSettings()
{
	for (auto& stream : m_streams)
...
void CFlowsheet2::UpdateToleranceSettings()
{
	for (auto& stream : m_streams)
...
const CDistributionsGrid* CFlowsheet2::GetDistributionsGrid() const
{
	return &m_grid;
}

CDistributionsGrid* CFlowsheet2::GetDistributionsGrid()
{
	return &m_grid;
}

const CParametersHolder* CFlowsheet2::GetParameters() const
{
	return &m_parameters;
}

CParametersHolder* CFlowsheet2::GetParameters()
{
	return &m_parameters;
}

bool CFlowsheet2::SaveToFile(CH5Handler& _h5File, const std::wstring& _fileName)
{
...
bool CFlowsheet2::LoadFromFile(CH5Handler& _h5File, const std::wstring& _fileName)
{
	if (_fileName.empty()) return false;
...
bool CFlowsheet2::LoadFromFile_v3(CH5Handler& _h5File, const std::string& _path)
{
	const std::string root = "/";
	...
	for (size_t i = 0; i < m_units.size(); ++i)
		for (size_t j = i + 1; j < m_units.size(); ++j)
			if (m_units[i]->GetUniqueID() == m_units[j]->GetUniqueID())
				m_units[j]->SetUniqueID(StringFunctions::GenerateUniqueKey(GetAllUnitsKeys()));
...
std::vector<std::string> CFlowsheet2::GetAllUnitsKeys() const
{
	std::vector<std::string> keys;
	for (const auto& unit : m_units)
		keys.push_back(unit->GetUniqueID());
	return keys;
}

std::vector<std::string> CFlowsheet2::GetAllStreamsKeys() const
{
	std::vector<std::string> keys;
