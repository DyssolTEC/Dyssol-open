/* Copyright (c) 2020, Dyssol Development Team. All rights reserved. This file
 * is part of Dyssol. See LICENSE file for license information. */
...
public:
  ...
  size_t m_iNormCompounds{}; // Norm vector for differences in phase compound
                             // fractions.
  size_t m_iNormDistr{};     // Norm vector for differences in distributions.

public:
  void CalculateResiduals(double _time, double *_vars, double *_ders,
                          double *_res, void *_unit) override;
  void ResultsHandler(double _time, double *_vars, double *_ders,
                      void *_unit) override;
};
...
public:
  ...
  std::vector<EDistrTypes>
      m_distributions; // All defined distributed parameters.
